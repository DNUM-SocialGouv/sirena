...
describe('parseCsvLine', () => {
  ...
  // Même règle que le découpage en enregistrements : hors début de champ, un guillemet est
  // une donnée. Sinon il avalerait le délimiteur suivant, le compte de champs cesserait de
  // correspondre à l'en-tête et la ligne disparaîtrait au compteur des illisibles.
  it('should treat a lone quote inside an unquoted field as data', () => {
    expect(parseCsvLine('01001;LE 6" PONT;01400', ';')).toEqual(['01001', 'LE 6" PONT', '01400']);
  });

  it('should keep doubled quotes inside an unquoted field', () => {
    expect(parseCsvLine('a;b""c;d', ';')).toEqual(['a', 'b""c', 'd']);
  });

  it('should not let a lone quote swallow the rest of the line', () => {
    expect(parseCsvLine('a,b"c,d', ',')).toEqual(['a', 'b"c', 'd']);
  });

  it('should keep reading fields after a quoted one closes', () => {
    expect(parseCsvLine('"a",b,"c"', ',')).toEqual(['a', 'b', 'c']);
  });
});
