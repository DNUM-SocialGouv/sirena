import { EnvSchema } from './env.schema.js';

const processEnv = {
  // PC
  PC_DOMAIN: process.env.PC_DOMAIN,
  PC_CLIENT_ID: process.env.PC_CLIENT_ID,
  PC_CLIENT_SECRET: process.env.PC_CLIENT_SECRET,
  PC_ID_TOKEN_SIGNED_RESPONSE_ALG: process.env.PC_ID_TOKEN_SIGNED_RESPONSE_ALG,
  PC_USERINFO_SIGNED_RESPONSE_ALG: process.env.PC_USERINFO_SIGNED_RESPONSE_ALG,
  PC_REDIRECT_URI: process.env.PC_REDIRECT_URI,
  // Auth
  AUTH_TOKEN_NAME: process.env.AUTH_TOKEN_NAME,
  REFRESH_TOKEN_NAME: process.env.REFRESH_TOKEN_NAME,
  IS_LOGGED_TOKEN_NAME: process.env.IS_LOGGED_TOKEN_NAME,
  AUTH_TOKEN_EXPIRATION: process.env.AUTH_TOKEN_EXPIRATION,
  REFRESH_TOKEN_EXPIRATION: process.env.REFRESH_TOKEN_EXPIRATION,
  AUTH_TOKEN_SECRET_KEY: process.env.AUTH_TOKEN_SECRET_KEY,
  REFRESH_TOKEN_SECRET_KEY: process.env.REFRESH_TOKEN_SECRET_KEY,
  // Frontend
  FRONTEND_URI: process.env.FRONTEND_URI,
  FRONTEND_REDIRECT_URI: process.env.FRONTEND_REDIRECT_URI,
  FRONTEND_REDIRECT_LOGIN_URI: process.env.FRONTEND_REDIRECT_LOGIN_URI,
  // DematSocial
  DEMAT_SOCIAL_API_URL: process.env.DEMAT_SOCIAL_API_URL,
  DEMAT_SOCIAL_API_TOKEN: process.env.DEMAT_SOCIAL_API_TOKEN,
  DEMAT_SOCIAL_API_DIRECTORY: process.env.DEMAT_SOCIAL_API_DIRECTORY,
  DEMAT_SOCIAL_INSTRUCTEUR_ID: process.env.DEMAT_SOCIAL_INSTRUCTEUR_ID,
  DEMAT_SOCIAL_IGNORED_DOSSIERS: process.env.DEMAT_SOCIAL_IGNORED_DOSSIERS,
  DEMAT_SOCIAL_FETCH_ALL_STATES: process.env.DEMAT_SOCIAL_FETCH_ALL_STATES,
  // Esante
  ANNUAIRE_SANTE_API_KEY: process.env.ANNUAIRE_SANTE_API_KEY,
  ANNUAIRE_SANTE_API_URL: process.env.ANNUAIRE_SANTE_API_URL,
  // BAN (Base Adresse Nationale)
  ADDRESS_API_URL: process.env.ADDRESS_API_URL,
  GEO_REFERENTIEL_COMMUNES_URL: process.env.GEO_REFERENTIEL_COMMUNES_URL,
  GEO_REFERENTIEL_POSTAL_URL: process.env.GEO_REFERENTIEL_POSTAL_URL,
  // Internal
  LOG_FORMAT: process.env.LOG_FORMAT,
  LOG_LEVEL: process.env.LOG_LEVEL,
  TRUSTED_IP_HEADERS: process.env.TRUSTED_IP_HEADERS,
  LOG_EXTRA_CONTEXT: process.env.LOG_EXTRA_CONTEXT,
  SUPER_ADMIN_LIST_EMAIL: process.env.SUPER_ADMIN_LIST_EMAIL,
  // Sentry
  SENTRY_ENABLED: process.env.SENTRY_ENABLED,
  SENTRY_DSN_BACKEND: process.env.SENTRY_DSN_BACKEND,
  // Minio
  S3_BUCKET_ACCESS_KEY: process.env.S3_BUCKET_ACCESS_KEY,
  S3_BUCKET_SECRET_KEY: process.env.S3_BUCKET_SECRET_KEY,
  S3_BUCKET_ENDPOINT: process.env.S3_BUCKET_ENDPOINT,
  S3_BUCKET_NAME: process.env.S3_BUCKET_NAME,
  S3_BUCKET_REGION: process.env.S3_BUCKET_REGION,
  S3_BUCKET_ROOT_DIR: process.env.S3_BUCKET_ROOT_DIR,
  S3_BUCKET_PORT: process.env.S3_BUCKET_PORT,
  S3_ENCRYPTION_KEY: process.env.S3_ENCRYPTION_KEY,
  CLAMAV_HOST: process.env.CLAMAV_HOST,
  CLAMAV_PORT: process.env.CLAMAV_PORT,
  // Tipimail
  TIPIMAIL_API_KEY: process.env.TIPIMAIL_API_KEY,
  TIPIMAIL_USER_ID: process.env.TIPIMAIL_USER_ID,
  TIPIMAIL_API_URL: process.env.TIPIMAIL_API_URL,
  TIPIMAIL_FROM_ADDRESS: process.env.TIPIMAIL_FROM_ADDRESS,
  TIPIMAIL_FROM_PERSONAL_NAME: process.env.TIPIMAIL_FROM_PERSONAL_NAME,
  TIPIMAIL_REDIRECT_ALL_TO: process.env.TIPIMAIL_REDIRECT_ALL_TO,
  TIPIMAIL_DISABLE_SENDING: process.env.TIPIMAIL_DISABLE_SENDING,
  // Cron
  CRON_DEMAT_SOCIAL: process.env.CRON_DEMAT_SOCIAL,
  CRON_RETRY_AFFECTATION: process.env.CRON_RETRY_AFFECTATION,
  CRON_RETRY_IMPORT_REQUETES: process.env.CRON_RETRY_IMPORT_REQUETES,
  CRON_MONITOR_REQUESTS: process.env.CRON_MONITOR_REQUESTS,
  CRON_QUEUE_UNPROCESSED_FILES: process.env.CRON_QUEUE_UNPROCESSED_FILES,
  CRON_FILE_INTEGRITY_CHECK: process.env.CRON_FILE_INTEGRITY_CHECK,
  CRON_PURGE_ACCESS_LOGS: process.env.CRON_PURGE_ACCESS_LOGS,
  CRON_SYNC_GEO_REFERENTIEL: process.env.CRON_SYNC_GEO_REFERENTIEL,
  ACCESS_LOG_RETENTION_DAYS: process.env.ACCESS_LOG_RETENTION_DAYS,
  // API Key
  API_KEY_HASH_SALT: process.env.API_KEY_HASH_SALT,
  // Redis
  REDIS_HOST: process.env.REDIS_HOST,
  REDIS_PORT: process.env.REDIS_PORT,
  REDIS_TLS: process.env.REDIS_TLS,
  REDIS_USERNAME: process.env.REDIS_USERNAME,
  REDIS_PASSWORD: process.env.REDIS_PASSWORD,
  // Monitoring
  MONITORING_PORT: process.env.MONITORING_PORT,
  WORKER_MONITORING_PORT: process.env.WORKER_MONITORING_PORT,
  // Affectation
  AFFECTATION_DEBUG: process.env.AFFECTATION_DEBUG,
  // Migration SIREC
  MARIADB_SIREC_HOST: process.env.MARIADB_SIREC_HOST,
  MARIADB_SIREC_PORT: process.env.MARIADB_SIREC_PORT,
  MARIADB_SIREC_DB: process.env.MARIADB_SIREC_DB,
  MARIADB_SIREC_USER: process.env.MARIADB_SIREC_USER,
  MARIADB_SIREC_PASSWORD: process.env.MARIADB_SIREC_PASSWORD,
  REDIS_MIGRATION_CONCURRENCY: process.env.REDIS_MIGRATION_CONCURRENCY,

  // Metabase
  METABASE_SITE_URL: process.env.METABASE_SITE_URL,
  METABASE_SECRET_KEY: process.env.METABASE_SECRET_KEY,
  METABASE_DASHBOARD_ID: process.env.METABASE_DASHBOARD_ID,
  METABASE_DASHBOARD_ID_ADMIN: process.env.METABASE_DASHBOARD_ID_ADMIN,
  // Environment
  APP_ENV: process.env.APP_ENV,
  // Migration bucket
  S3_MIGRATION_BUCKET_ACCESS_KEY: process.env.S3_MIGRATION_BUCKET_ACCESS_KEY,
  S3_MIGRATION_BUCKET_SECRET_KEY: process.env.S3_MIGRATION_BUCKET_SECRET_KEY,
  S3_MIGRATION_BUCKET_NAME: process.env.S3_MIGRATION_BUCKET_NAME,
};

const validateEnvVars = () => EnvSchema.parse(processEnv);

export const envVars = validateEnvVars();
