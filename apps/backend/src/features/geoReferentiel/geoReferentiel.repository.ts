import { Prisma, prisma } from '../../libs/prisma.js';
...
/** Une ligne de `VALUES`, chaque colonne typée `text` pour que Postgres n'ait rien à deviner. */
const sqlValuesRow = (values: (string | null)[]) =>
  Prisma.sql`(${Prisma.join(values.map((value) => Prisma.sql`${value}::text`))})`;

/**
 * Les mises à jour partent par lots, une requête par lot.
 *
 * Une boucle d'`update` Prisma ferait un aller-retour par ligne sur l'unique connexion de la
 * transaction. Le diff mensuel se compte en dizaines de lignes, mais pas le premier : sur un
 * environnement déjà alimenté par l'ancien `op:import:geodata`, `InseePostal.libelleAcheminement`
 * est NULL partout alors que la source le renseigne, si bien que la table entière ressort
 * comme modifiée. Plus de 35 000 allers-retours dépasseraient le délai de la transaction, qui
 * annulerait tout — et rejouerait le même échec chaque mois, sans jamais converger.
 *
 * `UPDATE ... FROM (VALUES ...)` ramène cela à un aller-retour par millier de lignes.
 * `updatedAt` est posé à la main : le SQL brut court-circuite le `@updatedAt` de Prisma.
 */
const updateCommunesBatch = async (tx: Prisma.TransactionClient, batch: CommuneRow[]) => {
  ...
  await tx.$executeRaw`
    UPDATE "Commune" AS target
    SET "comLib" = source."comLib",
        "metomerLib" = source."metomerLib",
        "ctcdCodeActuel" = source."ctcdCodeActuel",
        "ctcdLibActuel" = source."ctcdLibActuel",
        "dptCodeActuel" = source."dptCodeActuel",
        "dptLibActuel" = source."dptLibActuel",
        "regCodeActuel" = source."regCodeActuel",
        "regLibActuel" = source."regLibActuel",
        "updatedAt" = NOW()
    FROM (VALUES ${Prisma.join(values)}) AS source(
      "comCode", "comLib", "metomerLib", "ctcdCodeActuel", "ctcdLibActuel",
      "dptCodeActuel", "dptLibActuel", "regCodeActuel", "regLibActuel"
    )
    WHERE target."comCode" = source."comCode"
  `;
};

const updateInseePostalBatch = async (
  tx: Prisma.TransactionClient,
  batch: Array<{ id: string; row: InseePostalRow }>,
) => {
  const values = batch.map(({ id, row }) => sqlValuesRow([id, row.nomCommune, row.libelleAcheminement, row.ligne5]));

  await tx.$executeRaw`
    UPDATE "InseePostal" AS target
    SET "nomCommune" = source."nomCommune",
        "libelleAcheminement" = source."libelleAcheminement",
        "ligne5" = source."ligne5",
        "updatedAt" = NOW()
    FROM (VALUES ${Prisma.join(values)}) AS source("id", "nomCommune", "libelleAcheminement", "ligne5")
...
const writeCommunes = async (tx: Prisma.TransactionClient, diff: CommuneDiff) => {
  ...
  for (const batch of chunk(diff.toUpdate, GEO_GUARDS.BATCH_SIZE)) {
    await updateCommunesBatch(tx, batch);
  }
};
...
const writeInseePostal = async (
  ...
  for (const batch of chunk(diff.toUpdate, GEO_GUARDS.BATCH_SIZE)) {
    await updateInseePostalBatch(tx, batch);
  }
