...
export const GEO_GUARDS = {
   ...
   *
   * Les deux seuils sont cumulatifs : il faut dépasser le ratio *et* le volume absolu pour
   * que les suppressions soient écartées. Le second n'est qu'un plancher pour les bases peu
   * remplies, où 2 % ne représentent qu'une poignée de lignes ; au volume de production
   * (~35 500 couples) 2 % valent environ 700 lignes, donc c'est le ratio qui décide.
   */
   ...
   *
   * Ne pas monter au-delà de ~7 000 : les mises à jour passent par un `UPDATE ... FROM
   * (VALUES ...)` et Postgres plafonne une requête à 65 535 paramètres liés. Les communes en
   * lient 9 par ligne, ce qui fixe le maximum à 7 281 — `InseePostal`, avec 4 paramètres par
   * ligne, est bien plus loin du plafond. À 1 000 la marge est de sept fois.
   */
  BATCH_SIZE: 1_000,
