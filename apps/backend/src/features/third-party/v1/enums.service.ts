import {
  ...
  SIREC_ONLY_MALTRAITANCE_TYPE_IDS,
...
// SIREC-only maltraitance values are excluded: they are not accepted by the third-party FaitSchema, so the
// enum endpoint must not advertise values a third party would then be rejected for submitting.
export const getMaltraitanceTypeEnums = async () =>
  await prisma.maltraitanceTypeEnum.findMany({ where: { id: { notIn: SIREC_ONLY_MALTRAITANCE_TYPE_IDS } } });
