...
const MAX_CREATE_REQUETE_RETRIES = 5;
const CREATE_REQUETE_BACKOFF_MS = 100;
...
export const createRequeteFromThirdParty = async ({
  ...
  return await retryWithBackoff(
    ...
    {
      maxRetries: MAX_CREATE_REQUETE_RETRIES,
      backoffMs: CREATE_REQUETE_BACKOFF_MS,
      shouldRetry: (error) => isPrismaUniqueConstraintError(error),
      context: { service: 'createRequeteFromThirdParty', conflictField: 'id' },
    },
  );
};
