...
const byReclamationsSchema = z.object({
  ...
  mockFilePath: z.string().optional(),
});
...
const app = factoryWithLogs
  ...
  .post('/by-reclamations', zValidator('json', byReclamationsSchema), async (c) => {
    const { sirecIds, deleteIfExists, migrateFiles, mockFilePath } = c.req.valid('json');
    ...
    const queued = await addSirecIdsToQueue(sirecIds, deleteIfExists, migrateFiles, mockFilePath);
    return c.json({ queued });
  })

  .post('/by-services', zValidator('json', byServicesSchema), async (c) => {
    const { serviceIds, deleteIfExists, migrateFiles, mockFilePath } = c.req.valid('json');
    const ids = await fetchSirecIdsByServiceIds(serviceIds);
    const queued = await addSirecIdsToQueue(ids, deleteIfExists, migrateFiles, mockFilePath);
