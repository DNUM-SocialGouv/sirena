...
describe('auth.controller.ts Auth Endpoints', () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it('POST /logout should clear cookies and redirect to FRONTEND_REDIRECT_LOGIN_URI', async () => {
  ...
  it('POST /logout-proconnect deletes session and redirects to the endSessionUrl when getSession(token) and buildEndSessionUrl succeed', async () => {
    const fakeTokenValue = 'dummyRefreshToken123';
    const cookieHeader = `${envVars.REFRESH_TOKEN_NAME}=${fakeTokenValue}`;
  ...
  it('GET /callback should handle successful login, create user and redirect to FRONTEND_REDIRECT_URI', async () => {
