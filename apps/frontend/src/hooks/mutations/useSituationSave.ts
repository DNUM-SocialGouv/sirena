...
export const useSituationSave = ({
  ...
  const handleSave = async (
    ...
    // An HTTP failure is already toasted by handleRequestErrors; only a request that never got an answer is silent.
    const newFaitFileIds = await uploadFaitFiles(faitFiles).catch((error: unknown) => {
